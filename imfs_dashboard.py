...
TAIWAN_STOCK_UNIVERSE = {
    ...
    'Industrials & Materials': [
        ...
        ('1434', 'Formosa Taffeta','Formosa Taffeta'),
    ],
    'Consumer & Retail': [
...
def _get_cashflow_row(cf: pd.DataFrame, *keys) -> float:
    """Try multiple row-name variants; return 0 if none found."""
    for key in keys:
        if key in cf.index:
...
def run_full_imfs_valuation(ticker: str, company_name: str, sector: str):
    """
    Full IMFS valuation:
      Owner Earnings → regime-adjusted WACC → 5-yr DCF → MOS price
      Forensic flags: Piotroski proxy, Beneish accruals, Altman Z proxy
    """
    ...
    # ── Owner Earnings ────────────────────────────────────────────
    # Owner Earnings = Net Income + D&A − CapEx  (Buffett definition)
    if not cf.empty:
        ...
        # Fallback: free cash flow from info
    ...
    # Approximate cost of debt from interest coverage or default 3%
    ...
    growth_5y      = max(0.0, min(float(raw_growth), 0.15))  # cap 0–15%
    tg             = IMFS_Config.TERMINAL_GROWTH

    dcf_pv = 0.0
    for yr in range(1, 6):
        projected_oe = oe_per_share * (1 + growth_5y) ** yr
        dcf_pv += projected_oe / (1 + wacc) ** yr
    ...
    # Piotroski F-Score proxy (max 7 here)
    pf = sum([
        roe > 0,
        roa > 0,
        op_cashflow > 0,
        op_cashflow > net_inc_info,        # cash quality > accruals
        debt_to_equity < 100,
        current_ratio > 1.0,
    ...
    # Beneish M-Score proxy — accruals ratio
    accruals_ratio = ((net_inc_info - op_cashflow) / max(abs(net_inc_info), 1))
    beneish_flag   = "⚠️ Possible Earnings Inflation" if accruals_ratio > 0.5 else "✅ Clean Accruals"

    # Altman Z-Score proxy (simplified with available data)
    z_proxy = (roe * 2.0) + (current_ratio * 0.5) + (pb * 0.3)
    z_label = ("⚠️ Distress Zone" if z_proxy < 1.5
               else "🟡 Grey Zone" if z_proxy < 3.0
               else "✅ Safe Zone")

    # ── Regime Fit ───────────────────────────────────────────────
    regime_fit   = sector in current_sectors
    regime_label = ("✅ Sector fits current regime" if regime_fit
                    else f"⚠️ {sector} not preferred — current bias: {current_bias}")

    # ── Buy / Hold / Avoid Signal ────────────────────────────────
    if price > 0 and price <= mos_price:
        signal, signal_color = "BUY — Within MOS", "green"
    elif price > 0 and price <= intrinsic_value:
        signal, signal_color = "HOLD — Below Intrinsic", "orange"
    else:
        signal, signal_color = "AVOID — Above Intrinsic", "red"
    ...
    wacc_df = pd.DataFrame({
        "Component": ["Risk-Free Rate", "Beta × ERP", "Geo Buffer (TW)", "Regime Penalty", "→ Cost of Equity", "WACC (blended)"],
        "Value (bps)": [
            f"{rf*10000:.0f}",
            f"{beta * IMFS_Config.ERP / 100 * 10000:.0f}",
            f"{IMFS_Config.GEO_BUFFER}",
            f"{IMFS_Config.REGIME_PENALTY if _is_overheat_regime() else 0}",
            f"{cost_of_equity*10000:.0f}",
            f"{wacc*10000:.0f}",
        ],
        "Value (%)": [
            f"{rf*100:.2f}%",
    ...
    if not cf.empty:
        col_a.metric("Net Income", f"NT${net_income/1e9:.2f}B")
        col_b.metric("D&A", f"NT${da/1e9:.2f}B")
        col_c.metric("CapEx", f"NT${capex/1e9:.2f}B")
        col_d.metric("Owner Earnings", f"NT${owner_earnings/1e9:.2f}B")
    else:
        col_a.metric("Free Cash Flow (fallback)", f"NT${owner_earnings/1e9:.2f}B")
    ...
    fig_dcf = px.bar(pd.DataFrame(dcf_data), x="Component", y="NT$",
                     color="Component", title="DCF Components vs MOS Price")
    if price:
...
if page == "📊 Main Dashboard":
    ...
    st.table(pd.DataFrame({
        ...
        "Mitigation":  ["Quarterly PMI check", "+150 bps geo buffer", "+150 bps regime penalty", "Large-cap focus"],
    }))
...
# ====================== PAGE 2: STOCK PICKER & VALUATION ======================
elif page == "🏷️ Stock Picker & Valuation":
    ...
    search = st.text_input("Search ticker or company name (e.g. '2330' or 'TSMC' or 'Finance')", "")
    q = search.strip().lower()
    filtered = (
        [x for x in all_stocks if q in x["ticker"] or q in x["full"].lower()
    ...
    if not filtered:
        ...
        # Group by sector for the selectbox label
        options = [x["display"] for x in filtered]
        chosen_display = st.selectbox(
            f"Select from {len(filtered)} result(s)",
            options,
            help="Type in the search box above to narrow down",
        )
        chosen = next(x for x in filtered if x["display"] == chosen_display)

        # Show quick facts before running full valuation
        col_a, col_b, col_c = st.columns(3)
        col_a.markdown(f"**Ticker:** `{chosen['ticker']}.TW`")
        col_b.markdown(f"**Sector:** {chosen['sector']}")
        col_c.markdown(
            f"**Regime fit:** {'✅ Yes' if chosen['sector'] in current_sectors else '⚠️ No'}"
        )
...
# ====================== PAGE 3: TICKER QUICK VALUATION ======================
    ...
    if st.button("Fetch"):
        ...
        if info:
            short, full, sector = TICKER_LOOKUP.get(ticker_input, ("—", ticker_input, "Unknown"))
...
# ====================== PAGE 4: DYNAMIC STOCK SCANNER ======================
    ...
    if selected_sectors and scan_button:
        if "Quality" in current_bias:
            pe_t, pb_t, dy_min = 12, 1.0, 0.03
        else:
            pe_t, pb_t, dy_min = 18, 1.5, 0.02

        with st.spinner("Scanning Taiwan market..."):
            scanner = MarketScanner(selected_sectors)
            results = scanner.scan(pe_t, pb_t, dy_min)
